...
class WPLE_Admin
{
    ...
    public function wple_menu_page()
    {
        ...
        if ( FALSE === get_option( 'wple_version' ) ) {
            ...
            if ( version_compare( get_option( 'wple_version' ), '5.7.20', '<=' ) ) {
                delete_option( 'wple_plan_choose' );
                update_option( 'wple_version', WPLE_PLUGIN_VERSION );
            }
    ...
    <span><strong>Recommended:-</strong> Before enforcing HTTPS, We highly recommend taking a backup of your site using some good backup plugin like <strong>"Backup Bolt"</strong> - <a href="' . $pluginstallURL . '" target="_blank">Install & Activate Backup Bolt</a> | <a href="#" class="wple-backup-skip">Ignore</a></span>    
	  </div>';
        }
        ...
        if ( isset( $_POST['nc'] ) ) {
            ...
            foreach ( $httpch as $index => $ch ) {
                ...
                if ( !$check && $chfileexists ) {
                    ...
                    WPLE_Trait::wple_logger( 'Local file exists - Helping with HTTP challenge file', 'success', 'a' );
                    file_put_contents( $fpath . $ch['file'], trim( $ch['value'] ) );
                    //re-check once
                    $check = LEFunctions::checkHTTPChallenge(
                        $domain,
